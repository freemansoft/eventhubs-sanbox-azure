...
identity_metadata=$(az identity show --name $EVENTHUBS_ID_NAME --resource-group $AZURE_RESOURCE_GROUP)

EVENTHUBS_ID_NAME=$(jq -r ".name" <<< "$identity_metadata")
...
echo "UAI: $EVENTHUBS_ID_NAME principal: $EVENTHUBS_ID_PRINCIPAL_ID UAI id: $EVENTHUBS_ID_IDENTITY_ID "
